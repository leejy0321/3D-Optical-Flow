...
public class TrackCurve {
	...
	public float GetFraction(Vector3 position) {
		//Use a series of lines (from pathData). Also include rays cast from the start and end to infinity.

		//Determine which line segment position is closest to our target.
		...
		int numPieces = pathData.Length - 1;

		var closestFraction = 0f;
		var closestDistance = float.PositiveInfinity;

		var prevPos = pathData[0].pos;
		var prevPosFraction = 0f;

		//Debug.Log("Position is " + position);
		for (int i = 1; i <= numPieces; ++i) {
			var fraction = pathData[i].fraction;
			var pos = pathData[i].pos;

			var lineClosestFraction = NearestPartOfSegment(position, prevPos.position, pos.position);
			var lineDistance = DistanceFromSegment(position, prevPos.position, pos.position);
			...
			if (lineDistance < closestDistance) {
				...
				closestFraction = prevPosFraction + lineClosestFraction / (float)numPieces;
			}

			prevPosFraction = fraction;
			prevPos = pos;
		}
