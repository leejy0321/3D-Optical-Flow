...
public class Track : MonoBehaviour {
	...
	private MeshFilter ourMeshFilter;

	private MeshFilter MeshFilter {
		get {
			if (ourMeshFilter) return ourMeshFilter;
			return ourMeshFilter = GetComponent<MeshFilter>();
		}
	}
	...
	protected Mesh TryToFindGeneratedMesh() {
		...
		//Were we pasted/duplicated/Object.Instantiated? If so, we must make our own mesh.
	...
	private void FindOrCreateMesh() {
		generatedMesh = TryToFindGeneratedMesh();

		if (!generatedMesh) {
			//make one to call our own.
			generatedMesh = new Mesh();
			generatedMesh.name = "Generated Track";
		}
	}
	...
	/**
	 * Returns the length of this section of track (in local space), including any bends and curves of the track.
	 * This is the "ride" length of the track.
	 * To get linear distance use track.Distance.
	 */
	public float Length {
		get { return Curve.Length; }
	}

	/** Same as track.Length, but returns the length in world space. */
	public float LengthAbsolute {
		get {
			//note: using scale.x here, we could also use .y or .z as they all should be the same.
			//(We only support uniform scaling.)
			return Curve.Length * transform.lossyScale.x;
		}
	}

	/**
	 * Returns the distance between the start of the track and the end of the track (in local space).
	...
	/** Same as track.Distance, but in absolute/world space. */
	public float DistanceAbsolute {
		get { return endTransform.position.magnitude * transform.lossyScale.x; }
	}

	protected TrackCurve _curve;
	/**
	 * Returns the curve that backs this piece of track.
	 * Note that all data you can get from this curve is in the track-local coordinate space.
	 * (Use SimpleTransform.LocalToAbsolute(track.transform) to convert local values to global values.)
	 */
		...
		get {
			if (_curve == null) {
				try {
					...
					Debug.LogError("This track's end is coincident with the start, moving start and end apart.", gameObject);
	...
	public SimpleTransform NearestPointAbsolute(Vector3 pos) {
		...
		return NearestPoint(localPoint).LocalToAbsolute(transform);
	}
	...
	protected void GenerateMesh() {
		...
		if (gameObject.isStatic && Application.isPlaying && !isStartup) {
			Debug.LogWarning("Trying to update a mesh on a track marked as static. (A track's mesh can't be modified if it's static.)", this);
		}
#endif

		if (!generatedMesh) FindOrCreateMesh();
		if (MeshFilter.sharedMesh != generatedMesh) MeshFilter.sharedMesh = generatedMesh;
	...
	public Track GetNext(Track prev) {
		if (prev == PrevTrack) {
			...
			//We're not connected to to that one.
			Debug.LogWarning("Invalid: called Track.getNext with a \"previous\" track that's not actually connected", this);
			return null;
		}
	}
