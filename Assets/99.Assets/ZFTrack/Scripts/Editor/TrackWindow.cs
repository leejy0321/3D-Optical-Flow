...
public class TrackWindow : EditorWindow {
	...
	/**
	 * Direction the track is offset, relative to the start of the track (but in world scale/size).
	 */
	...
	public void OnEnable() {
		...
		aimGrid.selected = -1;
		aimGrid.names = new[] {
			new GUIContent("↶", "Bank Left"),
			new GUIContent("↑", "Pitch Up"),
			new GUIContent("↷", "Bank Right"),

			new GUIContent("←", "Turn Left"),
			new GUIContent("Build", "Move to next track section, building a new one if there is none"),
			new GUIContent("→", "Turn Right"),

			new GUIContent("Straight", "Straighten the current track section removing all curving and twisting"),
			new GUIContent("↓", "Pitch Down"),
			new GUIContent("Flat", "Make the end of the current section end perfectly level and quantize the ending direction to the current snapping angle."),
		};
		aimGrid.items = new[] {
		...
		lengthButtons.names = new[] {
			new GUIContent("Unbuild", "Delete the current track piece and move to the previous piece."),
			new GUIContent("Shorter", "Shorten the current track section"),
			new GUIContent("Longer", "Lengthen the current track section"),
			new GUIContent("Reverse", "Toggle which way is \"forward\" to this tool, allowing you to build from the end of the track instead of the front."),
		};
	...
	private void Regen() {
		...
		if (track) {
			...
			shim = new Vector3(0, 0, track.LengthAbsolute);
		}
	...
	private void ShimFix() {
		//Some versions of Unity like to let the next section overlap the sliders.
		//So apply a workaround.
	...
	private GUIContent[] navButtons = {
		new GUIContent("<<", "Select the previous track section"),
		new GUIContent(">>", "Select the next track section"),
	};
	private void RenderNavButtons() {
		...
		GUILayout.BeginHorizontal(GUIStyle.none);
		{
			...
			if (GUILayout.Button(navButtons[0], GUILayout.Height(ButtonHeight))) Defer(() => Nav(-1, shift));
			GUI.enabled = workDirection > 0 ? track.NextTrack : track.PrevTrack;
			if (GUILayout.Button(navButtons[1], GUILayout.Height(ButtonHeight))) Defer(() => Nav(1, shift));
			GUI.enabled = true;
		}
		GUILayout.EndHorizontal();
	}
	...
	public void AlignView() {
		...
		//Don't try to align view if we click on a project asset.
		if (!track.gameObject.activeInHierarchy) {
			//is it just disabled or not in the hierarchy at all?
			if (!track.gameObject.scene.isLoaded) return;
		}
