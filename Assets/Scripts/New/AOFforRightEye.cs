...
public class AOFforRightEye : MonoBehaviour
{
    ...
    private void Update()
    {
        if (AOFParams.instance.isEmptyForRight)
        {
            m_points.Clear();

            AOFParams.instance.isEmptyForRight = false;
        }
    }

    private void OnRenderImage(RenderTexture source, RenderTexture destination)
    {
        if (m_active && isLeft == false 
            && Camera.current.stereoActiveEye == Camera.MonoOrStereoscopicEye.Right)
        {
